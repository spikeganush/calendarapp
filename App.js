...
function App(props) {
  ...
  const createUser = async (collection, data) => {
    ...
    const ref = await setDoc(doc(FSdb, collection, data.id), data)
    console
      .log('User has added')
  ...
  const createTask = async (collection, data) => {
    ...
    const ref = await setDoc(
      doc(FSdb, `usertasks/${user?.uid}/documents/${new Date().getTime()} }`),
      data
    )
    //console.log( ref.id )
  }
  ...
  const getData = () => {
    ...
    const unsubscribe = onSnapshot(FSquery, (querySnapshot) => {
      ...
      setData(FSdata)
      console.log('FSdata in getData: ', FSdata)
    })
  }
  ...
  return (
    <Stack.Navigator>
      ...
      >
        {(props) => <EditTask {...props} auth={auth} />}
      </Stack.Screen>
    </Stack.Navigator>
  )
}
